...
import { GlobalContext } from '@src//context/GlobalContext'; // Importa tu contexto global
...
const ModalEditarVideo = ({ editarVideo, video, onClose, isOpen }) => {
  const [titulo, setTitulo] = useState(video.titulo);
  const [descripcion, setDescripcion] = useState(video.descripcion);
  const [url, setUrlVideo] = useState(video.url);
  const [foto, setFoto] = useState(video.foto);
  const [categoria, setCategoria] = useState(video.categoria);

  const { categorias } = useContext(GlobalContext); // Obtén las categorías del contexto global

  useEffect(() => {
    // Habilitar o deshabilitar el scroll del body según el estado del modal
    if (isOpen) {
